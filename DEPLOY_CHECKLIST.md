...
## 🔴 소유자 확인 누락 (IDOR) — 2026-09-15

인가 필터는 「로그인했나」 만 보고 「이 사람 것이 맞나」 는 API 마다 빠져 있었다.
**가입만 하면** 요청을 직접 만들어 아래가 전부 됐다. 화면에서 버튼을 숨기는 건 막는 게 아니다.

| 구멍 | 원인 |
|---|---|
| 남의 글 수정 | 수정 **화면**(GET)만 작성자 확인, 저장(PUT)은 확인 없음 |
| 남의 댓글 수정·삭제 | 확인 없음. 본문의 username 은 받기만 함 |
| 남의 이름으로 글·댓글 | 작성자를 요청 본문에서 받음 |
| **남의 계정 탈퇴** | `/memberdeletemail` 이 탈퇴 코드를 **응답으로 돌려줌** + `/memberdelete` 가 본문 username 을 믿음 |
| 남의 회원정보 수정 · 프로필 파일 삭제 | 대상을 `dto.email`, 지울 파일을 `dto.profileimage` 로 정함 |
| 남의 채팅방 조회·초대·방 이름 변경 | 참여자 확인 없음 |
| STOMP 남의 이름으로 발행 · 남의 방 구독 | sender 를 본문에서 받음, 구독 확인 없음 |
| 남이 올린 이미지 삭제 | `/deletecontentimage` 확인 없음 (프론트 미사용 → 엔드포인트 내림) |

`tools/OwnerCheck` 로 모았다. **사용자는 로그인 정보에서, 주인은 회원 id 로** 비교하고 아니면 403.
글·댓글에 복사돼 있는 username 문자열로 비교하지 않는다(나중에 어긋날 수 있다).
탈퇴는 프론트도 고쳤다 — 코드 비교를 서버로 옮겼고, 틀리면 200 대신 400.

검증: 시험 계정 둘로 위 경로를 전부 요청해 403/400, 정상 경로 200 을 확인했다(STOMP 포함).
배포 순서는 **백엔드 → 프론트**. 반대면 새 프론트가 옛 백엔드에 본문 없이 보내 탈퇴가 500 이 난다.
