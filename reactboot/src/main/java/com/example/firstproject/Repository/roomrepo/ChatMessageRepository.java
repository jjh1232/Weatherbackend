...
public interface ChatMessageRepository extends JpaRepository<chatmessage,Long> {
	...
	/* [2026-09-15] 입장·퇴장 시스템 메시지(member_id NULL)는 「대화」 로 치지 않는다.
	   마지막 글도, 안 읽은 수도 실제 대화만 본다. 시스템 메시지만 있는 방은 결과에 행이 없고,
	   프론트가 「아직 대화가 없어요」 로 보여준다. */
	@Query(value="SELECT m.room_id AS roomid,"
			...
			+ "FROM chatmessage Where room_id IN (:roomids) AND member_id IS NOT NULL "
